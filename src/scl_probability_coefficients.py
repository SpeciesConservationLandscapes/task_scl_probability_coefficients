...
class SCLProbabilityCoefficients(SCLTask):
    ...
    #     fit_pbso = minimize(self.neg_log_likelihood_int, param_guess, method="BFGS", options={"gtol": 1e-08})
    ...
    #     }
    #     p = {
    #         "coefs": pd.DataFrame(tmp, columns=["Parameter name", "Value", "Standard error"]),
    ...
    #         )
    #
    #     for j in range(0, len(self.df_signsurvey["detections"])):
    #         zeta[self.df_signsurvey["cell"][j] - 1, 1] = (
    #                 zeta[self.df_signsurvey["cell"][j] - 1, 1]
    #                 + (self.df_signsurvey["detections"][j]) * np.log(p_sign[self.df_signsurvey["SignSurveyID"][j] - 1])
    #                 + (self.df_signsurvey["NumberOfReplicatesSurveyed"][j] - self.df_signsurvey["detections"][j])
    #                 * np.log(1.0 - p_sign[self.df_signsurvey["SignSurveyID"][j] - 1])
    #         )
    #
    #     # TODO: make variable names more readable
    #     one = self.df_signsurvey[self.df_signsurvey["detections"] > 0]["cell"]
    ...
    # def predict_surface(self, par, griddata):
    ...
    #     temp = {"gridcellcode": griddata["gridcellcode"], "gridgridcells": gridcells, "condprob": cond_prob}
    #     prob_out = pd.DataFrame(temp, columns=["gridcellcode", "gridgridcells", "condprob"])
    #
    #     return prob_out

    def calc(self):
        # TODO: either we need to set these once per grid (filtering by grid id), and then combine the resulting grids,
        #  or the prob functions need to treat the grids separately internally
        # print(self.df_adhoc)
        # print(self.df_cameratrap_dep)
        # print(self.df_cameratrap_obs)
        print(self.df_signsurvey)
        # cell_totals = self.df_signsurvey.groupby(['GridCellCode'])['detections'].sum()
        ...
        for gridname in self.grids.keys():
            ...
            #     "covars.csv", encoding="utf-8", index_col="gridcellcode"
            # )
            # print(df_covars)
            ...
            # probs = self.predict_surface(m["coefs"]["Value"], df_covars)

            # def fake_prob_algorithm(f):
            #
            # # for each f in the fc, get # of observations that intersect
            # fake_prob_fc = cell_features.map()
            # # prob_images.append(ee.Image(1))
            # fake_prob = cell_features.reduceToImage(['probability'], ee.Reducer.max())
