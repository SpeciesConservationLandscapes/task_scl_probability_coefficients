...
def negLL_int(par):
    ...
    for j in range(0, len(sign["dets"])):
        zeta[sign["cell"][j] - 1, 1] = (
            zeta[sign["cell"][j] - 1, 1]
            + (sign["dets"][j]) * np.log(p_sign[sign["survey.id"][j] - 1])
            + (sign["reps"][j] - sign["dets"][j])
            * np.log(1.0 - p_sign[sign["survey.id"][j] - 1])
        )
...
griddata = pd.read_csv(
    r"C:\Users\Jamie\Documents\tiger\python_convert\model.inputs\griddata.csv"
)
CT = pd.read_csv(r"C:\Users\Jamie\Documents\tiger\python_convert\model.inputs\CT.csv")
po_data = pd.read_csv(
    r"C:\Users\Jamie\Documents\tiger\python_convert\model.inputs\po_data.csv"
)
sign = pd.read_csv(
    r"C:\Users\Jamie\Documents\tiger\python_convert\model.inputs\sign.csv"
)
Nx = 3
Nw = 3
Npsign = 1
